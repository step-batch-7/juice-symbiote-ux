...
describe("updateRecord", function() {
  ...
  it("it should first make a new array to the empId, then save the transaction if entry is made by new employee", function() {
    ...
    const fileContent = `[{"empId":"1","beverage":"orange","qty":"1","date":"${date.toJSON()}"}]`;
    const actualValue = updateRecord(
      userArgs,
      fileContent,
      path,
      ...
      function(path, content, encoder) {
        ...
        assert.strictEqual(
          content,
          `[{"empId":"1","beverage":"orange","qty":"1","date":"${date.toJSON()}"},{"empId":"2","beverage":"orange","qty":"1","date":"${date.toJSON()}"}]`
        );
        assert.strictEqual(encoder, "utf8");
      }
    );
  ...
  it("it should create transaction table object if transaction is entered by the employe", function() {
    ...
    const fileContent = `[{"empId":"1","beverage":"orange","qty":"1","date":"${date.toJSON()}"},{"empId":"2","beverage":"orange","qty":"1","date":"${date.toJSON()}"}]`;
    const actualValue = updateRecord(
      userArgs,
      fileContent,
      path,
      ...
      function(path, content, encoder) {
        ...
        assert.strictEqual(
          content,
          `[{"empId":"1","beverage":"orange","qty":"1","date":"${date.toJSON()}"},{"empId":"2","beverage":"orange","qty":"1","date":"${date.toJSON()}"},{"empId":"1","beverage":"lemon","qty":"1","date":"${date.toJSON()}"}]`
        );
      }
    );
    const expectedValue =
